...
(function(){
  ...
  function icon(value, extra='') {
    return `<span class="${cls('fn-ui-icon', extra)}" aria-hidden="true">${esc(value || '•')}</span>`;
  }

  function page(content='', extra='') {
    return `<div class="${cls('fn-ui-page','fn-ui-stack', extra)}">${safe(content)}</div>`;
  }
  ...
  function panel(opts={}) {
    ...
    return `<section class="${cls('fn-ui-panel', pad, opts.className)}">${head}${opts.children ? safe(opts.children) : ''}</section>`;
  }

  function tile(opts={}) {
    return `<div class="${cls('fn-ui-tile', opts.pad === false ? '' : 'fn-ui-tile-pad', opts.className)}">${safe(opts.children)}</div>`;
  }

  function featureGrid(items=[]) {
    return `<div class="fn-ui-feature-grid">${items.map(it => `<div class="fn-ui-feature"><span>${esc(it.icon || '•')}</span><div><b>${esc(it.title)}</b><small>${esc(it.text || '')}</small></div></div>`).join('')}</div>`;
  }
  ...
  function button(label, onclick='', opts={}) {
    ...
    return `<button type="button" class="${cls('fn-ui-button', kind, opts.className)}" ${onclick ? `onclick="${esc(onclick)}"` : ''}>${esc(label)}</button>`;
  }
  ...
  function field(opts={}) {
    ...
    let control = '';
    if (tag === 'select') {
      control = `<select ${opts.onchange ? `onchange="${esc(opts.onchange)}"` : ''}>${(opts.options||[]).map(o => `<option value="${esc(o.value)}" ${o.selected ? 'selected' : ''}>${esc(o.label)}</option>`).join('')}</select>`;
    } else if (tag === 'textarea') {
      control = `<textarea ${opts.oninput ? `oninput="${esc(opts.oninput)}"` : ''} placeholder="${esc(opts.placeholder || '')}">${esc(opts.value || '')}</textarea>`;
    } else {
      control = `<input id="${esc(opts.id || '')}" type="${esc(opts.inputType || 'text')}" value="${esc(opts.value || '')}" placeholder="${esc(opts.placeholder || '')}" ${opts.oninput ? `oninput="${esc(opts.oninput)}"` : ''} ${opts.onchange ? `onchange="${esc(opts.onchange)}"` : ''}>`;
    }
    return `<label class="fn-ui-field">${esc(opts.label || '')}${control}</label>`;
  }
  ...
  function chip(label, tone='') {
    return `<span class="${cls('fn-ui-chip', tone ? `fn-ui-chip-${tone}` : '')}">${safe(label)}</span>`;
  }

  function macroChips(m={}) {
    const out = [];
    if (m.kcal != null) out.push(chip(`🔥 ${esc(m.kcal)} kcal`, 'kcal'));
    if (m.prot != null) out.push(chip(`🍖 ${esc(m.prot)}g`, 'prot'));
    if (m.gluc != null) out.push(chip(`🍞 ${esc(m.gluc)}g`, 'gluc'));
    if (m.lip != null) out.push(chip(`🥑 ${esc(m.lip)}g`, 'lip'));
    return out.join('');
  }

  function dayStrip(days=[], opts={}) {
    return `<div class="fn-ui-day-strip">${days.map(d => `<button type="button" class="fn-ui-day-chip" title="${esc(d.title || '')}" ${d.onclick ? `onclick="${esc(d.onclick)}"` : ''}>
  ...
  function foodRow(opts={}) {
    return `<div class="fn-ui-row fn-ui-row--food fn-ui-food-row">
  ...
  function dayJournalCard(opts={}) {
    ...
    return `<details class="fn-ui-row fn-ui-row--history fn-ui-row--expandable fn-ui-day-card" id="${esc(opts.id || '')}">
  ...
  function metric(opts={}) {
    ...
    return `<div class="fn-ui-metric${tone}">
  ...
  function alertCard(opts={}) {
    ...
    return `<div class="fn-ui-alert fn-ui-status-card${tone}"><div class="fn-ui-alert-icon">${esc(opts.icon || '•')}</div><div><b>${esc(opts.title || '')}</b><p>${esc(opts.text || '')}</p></div></div>`;
  }
  ...
  function smartBadge(opts={}, key='') {
    ...
    return `<button type="button" class="fn-ui-smart-badge ${kind} ${macroCls}" data-recap-badge-key="${esc(key)}"${action}${style}>
